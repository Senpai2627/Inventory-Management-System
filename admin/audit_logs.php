...
$logType = isset($_GET['log_type']) ? trim($_GET['log_type']) : 'audit';
$ipFilter = isset($_GET['ip_filter']) ? trim($_GET['ip_filter']) : '';
...
if (!empty($search)) {
    if ($logType === 'page_views') {
        $where[] = "(p.page LIKE ? OR p.user_agent LIKE ? OR p.referrer LIKE ? OR p.ip_address LIKE ?)";
        $params = array_merge($params, ["%$search%", "%$search%", "%$search%", "%$search%"]);
        $types .= 'ssss';
    } else {
        $where[] = "(a.action LIKE ? OR a.table_name LIKE ? OR a.old_value LIKE ? OR a.new_value LIKE ? OR a.location LIKE ? OR a.ip_address LIKE ?)";
...
if (!empty($ipFilter)) {
    $where[] = ($logType === 'page_views' ? "p.ip_address LIKE ?" : "a.ip_address LIKE ?");
    $params[] = "%$ipFilter%";
    $types .= 's';
}
...
// Get distinct IPs for filter
$distinctIPs = $conn->query("SELECT DISTINCT ip_address FROM " . ($logType === 'page_views' ? 'page_views' : 'audit_logs') . " ORDER BY ip_address")->fetch_all(MYSQLI_ASSOC);
            ...
            <form method="get" action="">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <select class="form-select" id="ip_filter" name="ip_filter">
                            <option value="">All IPs</option>
                            <?php foreach ($distinctIPs as $ip): ?>
                                <option value="<?php echo htmlspecialchars($ip['ip_address']); ?>" 
                                    <?php echo $ipFilter === $ip['ip_address'] ? 'selected' : ''; ?>>
            ...
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        ...
                        <?php echo $logType === 'page_views' ? 'Page View Logs' : 'System Events Log'; ?>
                    </h5>
                    <?php if (!empty($search) || !empty($action) || $user > 0 || !empty($dateFrom) || !empty($dateTo) || !empty($ipFilter)): ?>
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo $logType === 'page_views' ? 'Page' : 'Event'; ?></th>
                                ...
                                <tr>
                                    ...
                                    <td class="ip-col">
                                        <span class="badge badge-ip" 
                                              ...
                                              title="<?php echo htmlspecialchars($log['ip_address']); ?>">
                                            <?php 
                                                $ip = htmlspecialchars($log['ip_address']);
                                                echo (strlen($ip) > 15 && strpos($ip, ':') !== false) ? 
                                                    substr($ip, 0, 8) . '...' . substr($ip, -5) : 
                                                    $ip;
                                            ?>
                                        </span>
                                        <i class="fas fa-info-circle ip-info-btn ms-1" 
                                           title="View IP info"
                                           onclick="showIpInfo('<?php echo htmlspecialchars($log['ip_address']); ?>')"></i>
                                    </td>
                                    ...
                                    <?php if ($logType === 'page_views'): ?>
                                        ...
                                        <td>
                                            <?php if ($log['referrer']): ?>
                                                <a href="<?php echo htmlspecialchars($log['referrer']); ?>" target="_blank" class="text-truncate d-block" style="max-width: 150px;">
                                                    <?php echo htmlspecialchars($log['referrer']); ?>
                                                </a>
                                            <?php endif; ?>
                                        </td>
                                    <?php endif; ?>
...
<div class="modal fade" id="ipInfoModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body" id="ipInfoContent">
...
<script>
    // Function to show IP info
    function showIpInfo(ip) {
        const modal = new bootstrap.Modal(document.getElementById('ipInfoModal'));
        document.getElementById('ipInfoContent').innerHTML = 'Loading IP information...';
        modal.show();
                ...
                let content = `
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>IP:</strong> ${data.ip || ip}</p>
                            <p><strong>Location:</strong> ${data.city || 'Unknown'}, ${data.state_prov || ''} ${data.country_name || ''}</p>
                            <p><strong>Country Code:</strong> ${data.country_code2 || data.country_code || 'N/A'}</p>
                            <p><strong>ISP:</strong> ${data.isp || data.organization || 'Unknown'}</p>
                        </div>
                        <div class="col-md-6">
                            <h6>Technical Details</h6>
                            <p><strong>Type:</strong> ${data.connection_type || 'Unknown'}</p>
                            <p><strong>ASN:</strong> ${data.asn || 'N/A'}</p>
                            <p><strong>VPN/Proxy:</strong> ${(data.proxy || data.vpn) ? 'Yes ⚠️' : 'No'}</p>
                            <p><strong>Coordinates:</strong> ${data.latitude || 'N/A'}, ${data.longitude || 'N/A'}</p>
                        </div>
                    </div>
                `;
                
                document.getElementById('ipInfoContent').innerHTML = content;
            })
            .catch(error => {
                document.getElementById('ipInfoContent').innerHTML = `
                    <div class="alert alert-danger">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.user-agent-col, .location-col, .ip-col .badge').forEach(cell => {
        ...
        // Add click handler for IP badges
        document.querySelectorAll('.ip-col .badge').forEach(badge => {
